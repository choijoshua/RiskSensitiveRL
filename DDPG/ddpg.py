    ...
    def learn(self, total_timesteps):
        ...
        while t < total_timesteps:
            ...
            for ep_t in range(1, self.max_timesteps_per_episode+1):
                ...
                ep_rews += reward.item()
                ep_len += 1
                self.memory.insert([obs, action, reward, mask, next_obs])
                ...
                Q_target = batch_rew + (1-batch_done) * self.gamma * self.target_critic(batch_next_obs, batch_next_action)

                Q = self.critic(batch_obs, batch_action)

                # Calculate Critic Loss
                critic_loss = nn.MSELoss()(Q.float(), Q_target.float())
